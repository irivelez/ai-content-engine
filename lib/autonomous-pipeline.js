...
async function curateContentFuel(rawItems) {
  ...
  for (let i = 0; i < deduped.length; i += BATCH_SIZE) {
    ...
    const prompt = `You are filtering viral AI content from X for a content creator who repurposes English X posts into Spanish LinkedIn content.

Your ONLY job: decide which posts are worth repurposing and explain WHY in 1 line. No creative writing. No hooks. No angles. Just filter and label.
...
For each item return JSON. If NOT worth it: {"keep":false}. If keeping:
{
  "keep": true,
  "repurposeScore": 8,
  "format": "listicle|how-to|thread|contrarian|before-after|tool-demo|case-study|framework",
  "whyKeep": "One sentence: why this specific post is gold for repurposing (English)"
}

## SCORING:
- 9-10: Viral format + step-by-step or deep actionable content + proven engagement
- 7-8: Strong format + useful content, clearly repurposable
- 5-6: Decent but needs heavy rework
- Below 6: reject

Only keep repurposeScore >= 7. Be ruthless. Max 10 items per batch.
Prioritize: real tutorials, tool demos, "I built X" threads, listicles with substance, contrarian takes with evidence.
Reject: vague hype, engagement bait, announcements with no actionable content.
