...
async function curateContentFuel(rawItems) {
  // Filter to items from content creators + content-broad tier + high-engagement broad
  const contentCandidates = rawItems.filter(item => {
    if (item.tier === 'creator' || item.tier === 'content-broad') return true;
    // Also include any item with very high engagement from any tier
  ...
  for (let i = 0; i < deduped.length; i += BATCH_SIZE) {
    ...
    const prompt = `You are a content strategist for Irina, a LATAM AI educator building her LinkedIn audience (Spanish-speaking professionals).

Analyze these ${batch.length} viral/popular AI posts from X. For each, score how REPURPOSABLE it is for LinkedIn content in Spanish.

Items:
${batch.map((item, j) => `[${j}] @${(item.author || '').replace('@', '')} | ${item.content?.slice(0, 300) || item.title} | ${item.likes || 0}♥ ${item.retweets || 0}🔁 ${item.views || 0}👁 | ${item.url}`).join('\n\n')}

For each item, return JSON. If NOT worth repurposing, return {"keep":false}. If keeping:
{
  "keep": true,
  "originalHook": "The hook/first line that made this go viral (in English)",
  "coreIdea": "The underlying idea worth repurposing (1 sentence)",
  "whyViral": "What made this resonate — format, emotion, specificity (1 sentence)",
  "repurposeScore": 8,
  "linkedinFormat": "listicle|how-to|contrarian-take|before-after|personal-experiment|tool-review|framework",
  "suggestedHookES": "Suggested LinkedIn hook in SPANISH that adapts the viral energy for LATAM professionals",
  "contentAngle": "Specific angle for Irina's LinkedIn post (English)",
  "tags": ["tools", "automation", "solopreneur"]
}

## SCORING (repurposeScore 1-10):
- 9-10: Viral format + actionable content + directly adaptable for LATAM audience + Irina has personal experience/angle
- 7-8: Strong format + useful content + adaptable with some localization
- 5-6: Interesting idea but needs significant rework or too niche for LATAM
- 3-4: Generic or too US/English-centric to adapt well
- 1-2: Pure engagement bait, no substance to repurpose

## RULES:
- Only keep items with repurposeScore >= 6
- The suggestedHookES must be SCROLL-STOPPING in Spanish, not a translation
- Think: "Would this format work on LinkedIn for a LATAM professional audience?"
- Prioritize: tool demos, step-by-step, "I built X", before/after, listicles, contrarian takes
- Tags from: tools, automation, solopreneur, productivity, no-code, claude, chatgpt, agents, business, career, freelance, startup
...
Return JSON array only.`;
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        for (let j = 0; j < Math.min(batch.length, analyses.length); j++) {
          const a = analyses[j];
          if (a.keep && (a.repurposeScore || 0) >= 6) {
            const raw = batch[j];
            allFuel.push({
              id: `cf-${Date.now()}-${allFuel.length}`,
              originalHook: a.originalHook || '',
              coreIdea: a.coreIdea || '',
              whyViral: a.whyViral || '',
              repurposeScore: Math.min(10, Math.max(1, a.repurposeScore || 5)),
              linkedinFormat: a.linkedinFormat || 'how-to',
              suggestedHookES: a.suggestedHookES || '',
...
async function runAutonomousPipeline() {
  ...
  try {
    ...
    const dailyData = {
      ...
      contentFuel,
      trends,
      stats: { rawFetched: rawCount, kept: items.length, contentFuelCount: contentFuel.length, sourceBreakdown, tierBreakdown },
    };
