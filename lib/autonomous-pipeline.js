...
async function curateEnterpriseSignals(rawItems) {
  // Filter to enterprise and latam tier items only
  const enterpriseItems = rawItems.filter(item =>
    item.tier === 'enterprise' || item.tier === 'latam'
  );
...
async function runAutonomousPipeline() {
  ...
  try {
    ...
    const enterpriseSignals = await curateEnterpriseSignals(raw);
    ...
    const dailyData = {
      ...
      enterpriseSignals,
      trends,
      stats: { rawFetched: rawCount, kept: items.length, contentFuelCount: contentFuel.length, enterpriseSignalCount: enterpriseSignals.length, sourceBreakdown, tierBreakdown },
    };
