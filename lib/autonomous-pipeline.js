...
async function detectTrends(todayItems) {
  ...
  if (days.length < 2) {
    ...
    return { emerging: [], consolidating: [], fading: [] };
  }
...
3. **fading**: Topics that had high presence (≥ 3 mentions/day) but dropped to ≤ 1 mention in last 2 days.
...
For fading, also include "peakDate" and "peakMentions".
...
Return JSON: {"emerging": [...], "consolidating": [...], "fading": [...]}`;
  ...
  try {
    ...
    if (match) {
      ...
      return {
        ...
        fading: (parsed.fading || []).slice(0, 3),
      };
    }
